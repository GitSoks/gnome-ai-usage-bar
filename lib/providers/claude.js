...
export class ClaudeProvider extends ProviderBase {
    ...
    async fetchQuota() {
        ...
        try {
            ...
            if (!token) {
                // Try to scan JSONL logs as last resort
                const localData = this._scanJsonlLogs();
                return {
                    installed: true,
                    authenticated: GLib.file_test(this._credsPath(), GLib.FileTest.EXISTS),
                    error: 'Token expired or missing — run claude once to re-authenticate',
    ...
    async _fetchOAuthUsage(token) {
        ...
        if (status === 401) return { error: 'Token rejected — run claude once to re-authenticate' };
