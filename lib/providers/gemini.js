...
export class GeminiProvider extends ProviderBase {
        ...
        try {
            ...
            if (!this._isTokenValid(creds)) {
                if (!creds.refresh_token) {
                    return { installed: true, authenticated: true, error: 'Token expired — run gemini once to refresh' };
                }
                try {
                    token = await this._refreshToken(creds);
                } catch (e) {
                    return { installed: true, authenticated: true, error: `Token refresh failed: ${e.message}` };
                }
            }
    ...
    async _fetchQuotaApi(token) {
        ...
        if (status === 401) return { error: 'Token rejected — run gemini once to re-authenticate' };
    ...
    _parseQuotaResponse(json) {
        ...
        if (!Array.isArray(buckets) || buckets.length === 0) {
            return { error: 'No quota buckets in response', rawResponse: json };
        }
        ...
        if (entries.length === 0) {
            return { error: 'Could not parse quota buckets', rawResponse: json };
        }
        ...
        if (!primary) return { error: 'No usable quota data', rawResponse: json };
