...
export class OpenCodeProvider extends ProviderBase {
    ...
    async fetchQuota() {
        ...
        try {
            ...
            if (goMode && this._settings.get_boolean('opencode-go-auto-fetch')) {
                let workspaceId = this._settings.get_string('opencode-workspace-id');
                let authCookie = this._settings.get_string('opencode-auth-cookie');

                // Auto-discover from browser if not manually configured
                if (!workspaceId || !authCookie) {
                    const discovered = await this._discoverCredentials();
                    if (discovered?.workspaceId) workspaceId = discovered.workspaceId;
                    if (discovered?.authCookie) authCookie = discovered.authCookie;
                }

                if (workspaceId && authCookie) {
                    try {
                        const scrapeResult = await this._scrapeGoPage(workspaceId, authCookie);
                        if (scrapeResult) {
                            // Persist discovered credentials so we don't need to re-extract every time
                            if (!this._settings.get_string('opencode-workspace-id'))
                                this._settings.set_string('opencode-workspace-id', workspaceId);
                            if (!this._settings.get_string('opencode-auth-cookie'))
                                this._settings.set_string('opencode-auth-cookie', authCookie);

                            // Merge local DB cost/call stats as supplementary data
    ...
    _parseGoHtml(html) {
        ...
        const extractWindow = (name) => {
            // Build a flexible pattern that captures any key:value pairs within the braces
            ...
            // resetInSec: rolling windows (5h, weekly)
            ...
            // resetDate: ISO date string billing cycle (e.g. "2026-06-01T00:00:00Z")
            if (!resetAt) {
            ...
            // resetTimestamp: Unix epoch seconds
            if (!resetAt) {
        ...
        // Billing cycles typically reset on the 1st of each month; use as fallback
