...
export class CopilotProvider extends ProviderBase {
    ...
    _parseUsageResponse(json) {
        // Real response shape (confirmed from API):
        // {
        //   quota_reset_date_utc: "2026-06-01T00:00:00.000Z",  ← top-level, shared by all snapshots
        //   quota_snapshots: {
        //     premium_interactions: { remaining: 276, entitlement: 300, percent_remaining: 92,
        //                             unlimited: false, overage_count: 0, overage_permitted: true, … },
        //     chat:        { unlimited: true, … },
        //     completions: { unlimited: true, … },
        //   }
        // }
        const snapshots = json.quota_snapshots ?? json.quotaSnapshots ?? {};

        // Reset date lives at top level, not inside each snapshot
        const resetDateStr = json.quota_reset_date_utc ?? json.quotaResetDateUtc ?? null;
        const globalResetAt = resetDateStr ? new Date(resetDateStr) : null;

        const makeWindow = (s, label) => {
            if (!s) return null;
            if (s.unlimited === true) return null;
            // Skip snapshots with no configured quota (entitlement=0, has_quota=false)
            const entitlement = s.entitlement ?? s.quota_entitlement ?? 0;
            if (entitlement === 0) return null;

            const pct = s.percent_remaining ?? s.percentRemaining ?? null;
            if (pct === null) return null;

            const remaining  = s.remaining  ?? Math.round(pct / 100 * entitlement);
            const overageCnt = s.overage_count ?? s.overageCount ?? 0;

            // Build a compact count string for the detail area
            let countText = `${remaining}/${entitlement}`;
            if (overageCnt > 0) countText += ` +${overageCnt} overage`;
        ...
        const premium     = snapshots.premium_interactions ?? snapshots.premiumInteractions;
        const chat        = snapshots.chat;
        const completions = snapshots.completions;

        const windows = [
            makeWindow(premium,     'Premium'),
            makeWindow(chat,        'Chat'),
            makeWindow(completions, 'Completions'),
        ].filter(Boolean);

        if (windows.length === 0)
            return { error: 'No active quota snapshots in response' };

        const result = {};
        if (windows[0]) result.session = windows[0];
        if (windows[1]) result.weekly  = windows[1];
        if (windows[2]) result.monthly = windows[2];
        return result;
    }
}
